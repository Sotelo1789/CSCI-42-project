...
def my_requests_view(request):
    ...
    """
    prlist = PurchaseRequest.objects.filter(buyer=request.user).order_by("pk") #removed the status open so the user can view the offers of purchase request that are done
    """Ensure all purchase requests are truly open"""
    for pr in prlist:
        if pr.closing_deadline <= timezone.now():
            pr.status = "closed"
            pr.save()
    prlist = PurchaseRequest.objects.filter(buyer=request.user).order_by("pk") # same as the one above
...
def view_offers_view(request, pk):
    ...
    if purchase_request.status == "open" and purchase_request.closing_deadline <= timezone.now():
        purchase_request.status = "closed"
        purchase_request.save()

    # Get all offers related to that purchase request
    offers = Offer.objects.filter(
        purchase_request=purchase_request
    ).select_related('seller', 'seller__business_profile').order_by('-submitted_at')
    ...
    return render(request, 'purchase_requests/view_offers.html', context)


@login_required
def accept_offer_view(request, pk, offer_pk):
    purchase_request = get_object_or_404(PurchaseRequest, pk=pk, buyer=request.user)
    offer = get_object_or_404(Offer, pk=offer_pk, purchase_request=purchase_request)

    if not purchase_request.is_past_deadline:
        messages.error(request, "You can only select a winner after the deadline.")
        return redirect('purchase_requests:view_offers', pk=purchase_request.pk)

    # Prevent selecting another winner if already completed
    if pr.status == "completed":
        return redirect('purchase_requests:view_offers', pk=pr.pk)
